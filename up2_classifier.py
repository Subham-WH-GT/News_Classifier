...
for post in reddit_posts:
    ...
    bert_label, bert_conf = classify_bert(input_text)
    llm_label, llm_reason = classify_llm(post)

    # --- Decision Logic ---
    votes = [deberta_label, bert_label, llm_label]
    final_label = max(set(votes), key=votes.count)
    final_confidence = round(np.mean([
        deberta_conf if deberta_label == final_label else 0,
        bert_conf if bert_label == final_label else 0,
        0.95 if llm_label == final_label else 0  # assign LLM a fixed weight if it agrees
    ]), 4)
...
print("\n✅ All results saved to verdicts.json")